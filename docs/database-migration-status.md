...
Verificacion actualizada el 2026-06-27 contra la base local configurada por `backend/src/config/env.js`.
...
| `007-project-scenario-versions.sql`  | Tabla `versiones_escenarios`, claves a `proyectos` y `usuarios`, e indices de historial presentes                                         | Aplicada     |
...
`T008` y la base de versionado de `002` quedan cerrados para el entorno local. Para produccion o una base remota, repetir esta verificacion antes de crear migraciones nuevas o tocar versionado de escenarios.
